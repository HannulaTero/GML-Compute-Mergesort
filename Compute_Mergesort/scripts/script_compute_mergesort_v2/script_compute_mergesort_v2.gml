/*
  ...
  This is in all ways similar to V1, but the uniforms are precreated for all possible combinations.
  This allows reducing CPU-GPU data movement, and should make things bit faster.
  Uniforms are selected with dynamic index within uniform buffer. 
    -> Note that dynamic index requires alignment of 256 (some hardware may support smaller).
  
*/
/// @func ComputeMergesortV2(_params);
/// @desc Does parallel mergesort with compute shaders. 
/// @param {Struct} _params Check constructor for accepted parameters.
function ComputeMergesortV2(_params={}) constructor
{
