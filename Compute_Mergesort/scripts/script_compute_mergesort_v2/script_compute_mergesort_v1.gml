...
function ComputeMergesortV1(_params={}) constructor
{
  ...
  // For to specify struct and how to access specific element.
  ...
  self.shaderModule = device.createShaderModule({ 
    ...
    code: ComputeMergesortSimple.Replace({
        WORKGROUP_SIZE: string(workgroupSize),
        IMPORTS : _imports,
        DATATYPE : self.dtype,
        ACCESS : _access,
      }, ComputeMergesortSimple.shaderSource
    )
  });
  ...
  for(var i = 0; i < _maxPasses; i++)
  {
    ...
    // buffer_seek(_buffer, buffer_seek_start, i * _alignment);
    // show_debug_message($"===================================");
    // show_debug_message($"aligment   : {i * _alignment}");
    // show_debug_message($"groupShift : {buffer_read(_buffer, buffer_u32)}");
    // show_debug_message($"localShift : {buffer_read(_buffer, buffer_u32)}");
    // show_debug_message($"localSize  : {buffer_read(_buffer, buffer_s32)}");
    // show_debug_message($"passIndex  : {buffer_read(_buffer, buffer_u32)}");
  }
  ...
  static shaderSource = (@'
    ...
    fn computeMerge(@builtin(global_invocation_id) id : vec3u)
    {
      ...
      // Invocation handles finding LHS value position RHS list.
      if (id.y == 0)
      {
      ...
      // Invocation handles finding RHS value position LHS list.
      else 
      {
